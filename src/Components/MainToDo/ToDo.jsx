...
function ToDo(done, toDoBody, deleteToDo, index, id, updateToggle) {

    const [ isLightTheme ] = useContext(Context)
    const [isCompleted, setIsCompleted ] = useState(done.done)

    function handleToggle() {
        const opposite = !isCompleted;
        setIsCompleted(opposite)
    ...
    return <div className="todo-card" data-theme={isLightTheme}>
        ...
        {isCompleted && <svg xmlns="http://www.w3.org/2000/svg" width="11" height="9"><path fill="none" stroke="#FFF" stroke-width="2" d="M1 4.304L3.696 7l6-6"/></svg>}
        </button>
